...
__ApiLevel=android-21 # The minimum platform for arm64 is API level 21
__BuildArch=arm64
__AndroidArch=aarch64
__AndroidToolchain=aarch64-linux-android-5
...
__NDK_Dir="$__Android_Cross_Dir/crystax-ndk-$__NDK_Version"
...
if [ ! -d $__NDK_Dir ]; then
    echo Downloading the NDK into $__Android_Cross_Dir
    mkdir -p $__Android_Cross_Dir
    wget -nv -nc --show-progress https://www.crystax.net/download/crystax-ndk-$__NDK_Version-linux-x86_64.tar.xz -O $__Android_Cross_Dir/crystax-ndk-$__NDK_Version-linux-x86_64.tar.xz

# Unpacking crystax NDK
    echo Unpacking the NDK into $__Android_Cross_Dir
    tar -xf $__Android_Cross_Dir/crystax-ndk-$__NDK_Version-linux-x86_64.tar.xz -C $__Android_Cross_Dir
#    mv $__Android_Cross_Dir/android-platform-ndk-crystax-ndk-$__NDK_Version/* $__Android_Cross_Dir/crystax-ndk-$__NDK_Version/
fi
...
$__NDK_Dir/build/tools/make-standalone-toolchain.sh --toolchain=$__AndroidToolchain --arch=$__BuildArch --llvm-version=3.7 --install-dir=$__ToolchainDir
...
# Android specific support packages
...
# CoreFX and CoreCLR specific packages
...
echo "Now run:"
echo "CONFIG_DIR=`realpath cross/android/arm64` ROOTFS_DIR=`realpath /build/core/core-setup/cross/android-rootfs/toolchain/arm64/sysroot` ./run.sh build-native -CrossBuild=True -DistroRid=android.21-arm64 -SkipTests=True -Architecture=arm64 -TargetArchitecture=arm64 -Project=build.proj"